...
@Service
public class GetSendListServiceImpl implements GetSendListService {
    @Autowired
    ...
    public List<msg> getlist() {
        ...
        List<Messages> listmsg = messagesMapper.selectMsgBySendId(user.getId());
        List<msg> res=new ArrayList<>();
        for(Messages m:listmsg) {
            String txt=m.getMessagecontent();
            Date time=m.getTimestamp();
            String name=userMapper.selectById(m.getRecipientid()).getUsername();
